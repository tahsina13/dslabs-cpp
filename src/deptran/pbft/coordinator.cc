...
CoordinatorPbft::CoordinatorPbft(uint32_t coo_id,
                                             int32_t benchmark,
...
bool CoordinatorPbft::IsLeader() {
   return this->svr_->IsLeader() ;
}

bool CoordinatorPbft::IsFPGALeader() {
   return this->svr_->IsFPGALeader() ;
}

void CoordinatorPbft::Submit(shared_ptr<Marshallable>& cmd,
...
void CoordinatorPbft::AppendEntries() {
    std::lock_guard<std::recursive_mutex> lock(mtx_);
...
void CoordinatorPbft::LeaderLearn() {
    std::lock_guard<std::recursive_mutex> lock(mtx_);
    commit_callback_();
    verify(phase_ == Phase::COMMIT);
    GotoNextPhase();
...
void CoordinatorPbft::GotoNextPhase() {
  int n_phase = 4;
